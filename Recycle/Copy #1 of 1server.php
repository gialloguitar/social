...
//set_time_limit(100);	//Время выполнения скрипта ограничено 180 секундами
...
while ($server_state) {
    ...
    $read = $connects;
    $read []= $socket;
    $write = $except = null;  //обязательные массивы потоков для stream_select
	//print_r($read);   //выводит вместе с сессией сервера
	//echo count($read);

    if (!stream_select($read, $write, $except, null)) {//ожидаем сокеты доступные для чтения (без таймаута)
        break;   //прекращение цикла при закрытии прослушиваемого сокета
    }
    if (in_array($socket, $read)) {//есть новое соединение то обязательно делаем handshake
        ...
        if (($connect = stream_socket_accept($socket, -1)) && $info = handshake($connect)) {
			...
			echo "connect=".$connect.", ip=".$info['ip']."<br />OK<br />"; 
			$connects[] = $connect;//добавляем его в список необходимых для обработки
            onOpen($connect, $info);//вызываем пользовательский сценарий
        }
        unset($read[ array_search($socket, $read) ]);
		//print_r($read); //вывод без учета сессии сервера
    }
    ...
    foreach($read as $connect) {//обрабатываем все соединения	  
        ...
        onMessage($connect, $data, $connects);//вызываем пользовательский сценарий
    }
 /* if(isset($_GET['OFF'])) {
	           fclose($socket);
	           echo "exit <br />\r\n";
...
function onOpen($connect, $info) {
        $f = "Client".$info['ip']."Connect<br>\n";
	    echo $connect." open OK with IP: ".$info['ip']."<br />\n";
 
		//fwrite($connect, encode('Privet')); //echo функция ответа 
}

function onClose($connect) {
    echo $connect." close OK<br />\n";
}

function onMessage($connect, $data, $connects) {
        //print_r($connects);
		$f = decode($data); 
